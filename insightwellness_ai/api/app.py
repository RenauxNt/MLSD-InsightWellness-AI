...
EXPECTED_MODEL_SCHEMA = {
    ...
    "MTRANS_walking": {"description": "Main transport is walking (0=No, 1=Yes). Max ONE MTRANS variable can be 1.", "valid_values": [0, 1]}
}
...
def predict():
    ...
    try:
        ...
        for key, value in data.items():
            if key in EXPECTED_MODEL_SCHEMA:
                ...
                if isinstance(valid_values, list) and value not in valid_values:
                    return jsonify({"error": f"Invalid input for '{key}'. Expected one of {valid_values}, got {value}."}), 400
                
                elif isinstance(valid_values, str) and "Numeric" in valid_values:
                    if not isinstance(value, (int, float)):
                        return jsonify({"error": f"Invalid type for '{key}'. Expected number, got {type(value).__name__}."}), 400
        ...
        if mtrans_sum > 1:
            return jsonify({"error": "Invalid transportation data. Max ONE main MTRANS mode allowed."}), 400
