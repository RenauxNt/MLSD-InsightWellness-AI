...
### Weight and height removal.

We decided to remove the weight and the height features. The project switch to an early warning system for obesity based only on lifestyle. The BMI rule only detect obesity after the weight gain has occurred. The goal of our approach is to detect persons at risk before the weight gain.
