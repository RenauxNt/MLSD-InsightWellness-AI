...
- **`ci.yml`** — on every pull request: pre-commit (ruff lint + format) and the 31-test suite with coverage. Nothing deploys from a PR.
- **`deploy.yml`** — on merge to `main`: detects which service's files changed (path filters), re-runs the test gate, then builds, deploys, and **health-checks** only the affected Cloud Run services via the reusable `deploy-cloud-run.yml`.
- **`vertex-pipeline.yml`** — on merge to `main` touching training code or config: rebuilds the pipeline base image and re-runs the Vertex pipeline (continuous training).

Configuration (project ID, region, bucket) lives in GitHub repository variables and reaches the containers as environment variables — no cloud identifiers hardcoded in the codebase.
...
uv run pytest tests/ --cov=insightwellness_ai    # test suite (offline, ~1s)
uv run python -m insightwellness_ai.api.app       # API on :8080
uv run streamlit run insightwellness_ai/dashboard/streamlit_app.py
```
